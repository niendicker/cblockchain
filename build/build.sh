...
bin_output="${bin_output_dir}/bcNode" 
...
#?Debuggin version
echo g++ -Wall -g -o "${bin_output}_dbg.bin" "${sourceFiles[@]}" -lcrypto
     g++ -Wall -g -o "${bin_output}_dbg.bin" "${sourceFiles[@]}" -lcrypto && \
echo "X86_Linux: Debuggin version copilled successful"
echo "------------------------------------------------"

#?Profilling version 
#echo g++ -Wall -pg -o "${bin_output}_prof.bin" "${sourceFiles[@]}" -lssl -lcrypto
#     g++ -Wall -pg -o "${bin_output}_prof.bin" "${sourceFiles[@]}" -lssl -lcrypto && \
#echo "X86_Linux: Profilling version copilled successful"
#echo "------------------------------------------------"
#
##?Production version - DO NOT DEFINE NDEBUG TO KEEP ASSERTS()
#echo g++ -D QUIET_OUTPUT -Wall -o "${bin_output}.bin" "${sourceFiles[@]}" -lssl -lcrypto
#     g++ -D QUIET_OUTPUT -Wall -o "${bin_output}.bin" "${sourceFiles[@]}" -lssl -lcrypto && \
